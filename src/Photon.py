...
engine.setProperty('voice', voices[0].id)  #Sets the speech engine to use the first voice in the list of available voices.
...
def wish():
    ...
    reply("I am photon, how may I help you?")
...
def calculator(command):
    try:
        ...
        command = command.replace('photon', '').replace('calculate', '').strip()
...
def metric_converter(command):
    try:
        # Normalize input
        command = command.lower().replace("photon", "").replace("convert", "").strip()
