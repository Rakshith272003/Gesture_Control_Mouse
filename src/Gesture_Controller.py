...
import math #It is used for calculating the distance between the hand landmarks (Pinching and Spreading)
from enum import IntEnum #Eneumartions are used for numbering and indexing (Gesture types)
from ctypes import cast, POINTER# These are the tools makes python program to talk directly with computers hardware
from comtypes import CLSCTX_ALL#COM-Component object model
from pycaw.pycaw import AudioUtilities, IAudioEndpointVolume # used for audio control of the sysytem based on hand Gesture
from google.protobuf.json_format import MessageToDict #used for converting MediaPipe results into JSON like formats for easir manipulation
import screen_brightness_control as sbcontrol #used for adjusting system brightness 

pyautogui.FAILSAFE = False#If it is set to true and cursor moves to(0,0) script halts abruptly
mp_drawing = mp.solutions.drawing_utils# detected hand landmarks by drawing them on video frames.
mp_hands = mp.solutions.hands#detect and track hand gestures and landmarks.

# Gesture Encodings 
class Gest(IntEnum):
    # Binary Encoded
    """
    Enum for mapping all hand gesture to binary encoded number.
    """
...
class HLabel(IntEnum):#asily classify or tag which hand is performing an action 
    MINOR = 0#non-dominant hand 
    MAJOR = 1#dominant hand

# Convert Mediapipe Landmarks to recognizable Gestures
class HandRecog:
    
    
    def __init__(self, hand_label):#Constructors
        
        self.finger = 0#Initializes the state of fingers to 0 (no finger open).
        self.ori_gesture = Gest.PALM#Sets the initial gesture to PALM.
        self.prev_gesture = Gest.PALM# Tracks the previous gesture for handling gesture changes.
        self.frame_count = 0#ensures for consistency over certain number of frames before it recgongsine next stable gesture(Stablization) 
        self.hand_result = None#stores the land marks in 3D coordinates 
        self.hand_label = hand_label#it is used for dominant and non-dominant
    
    def update_hand_result(self, hand_result):
        self.hand_result = hand_result#effectively storing the new hand landmarks

    def get_signed_dist(self, point):#calculates the signed distance between two points (landmarks) on the hand
        #A list or tuple with two elements, representing the indices of two hand landmarks (points) whose distance needs to be calculated
        sign = -1#sign variable is initialized to -1
        ...
        dist = math.sqrt(dist)
        return dist*sign#adding a Directional Sign
    
    def get_dist(self, point):#Detecting Gestures ,Pinch FDetection,Validating Grstures States
    ...
    def get_dz(self,point):#calaculate z-coordinates(measuring depth information used for )
        #Validating Finger Alignment,Distinguishing Open and Closed Gestures
        return abs(self.hand_result.landmark[point[0]].z - self.hand_result.landmark[point[1]].z)
    
    # Function to find Gesture Encoding using current finger_state.
    # Finger_state: 1 if finger is open, else 0
    def set_finger_state(self):
        #Ensures the method is called only if hand landmarks are available.
        if self.hand_result == None:
            return

        points = [[8,5,0],[12,9,0],[16,13,0],[20,17,0]]#(Index,Middle,Ring,Pinky)
        #[Tip Landmark (8, 12, 16, 20):Base Landmark (5, 9, 13, 17):Wrist Landmark (0)]
        #Finger Encoding Initialization
        self.finger = 0#no fingers being detected or all fingers being closed
        self.finger = self.finger | 0 #thumb
        for idx,point in enumerate(points):
            
            dist = self.get_signed_dist(point[:2])#represent the landmarks of two jointspoint = [8, 5, 0], so point[:2] = [8, 5]
            dist2 = self.get_signed_dist(point[1:])# represents the next two finger joints (e.g., index and middle)[8, 5, 0], point[1:] = [5, 0]
